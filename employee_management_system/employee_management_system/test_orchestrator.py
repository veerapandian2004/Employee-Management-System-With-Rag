...
class TestChatOrchestrator(unittest.TestCase):
	...
	# -------------------------------------------------------------
	# 15. CHAT SESSION CREATION & ANTI-DUPLICATION
	# -------------------------------------------------------------
	def test_15_chat_session_anti_duplication(self):
		frappe.set_user("Administrator")
		# Clean up any existing empty sessions for Administrator
		sessions = get_chat_sessions()
		for s in sessions:
			if s.get("title") == "New Chat":
				delete_chat_session(s["name"])

		# 1. First call creates a new session
		s1 = create_chat_session("New Chat")
		self.assertIsNotNone(s1.get("name"))

		# 2. Second call with empty session should REUSE s1, NOT create a duplicate
		s2 = create_chat_session("New Chat")
		self.assertEqual(s1["name"], s2["name"], "Should reuse the existing empty chat session")

		# 3. Third call should still return s1
		s3 = create_chat_session()
		self.assertEqual(s1["name"], s3["name"], "Default title should also reuse empty session")

		# 4. If title is updated to a custom name, it updates the existing session
		s4 = create_chat_session("Engineering Query")
		self.assertEqual(s1["name"], s4["name"], "Should update title of empty session")
		self.assertEqual(s4["title"], "Engineering Query")

		# Clean up
		delete_chat_session(s1["name"])
